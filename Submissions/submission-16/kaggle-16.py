...
###################################################################################################
# Let's check the most correlated features to SalePrice and make some transformations to them
temporal_train_features = features.loc['train'].select_dtypes(include=[np.number])
temporal_train_features = pd.concat([temporal_train_features, train_labels], axis=1)

correlated = temporal_train_features.corr()
correlated.sort_values(["SalePrice"], ascending = False, inplace = True)
print("\nMost correlated features to SalePrice:")
print(correlated.SalePrice[0:10])

# Now we'll get some new features
...
features['SquaredGrLivArea'] = features['GrLivArea'] ** 2
features['CubedGrLivArea'] = features['GrLivArea'] ** 3

features['SquaredExterQual'] = features['ExterQual'] ** 2
features['CubedExterQual'] = features['ExterQual'] ** 3

features['SquaredGarageCars'] = features['GarageCars'] ** 2
features['CubedGarageCars'] = features['GarageCars'] ** 3
###################################################################################################
...
categorical_features = ('FireplaceQu', 'SimplifiedFireplaceQu', 'BsmtQual', 'SimplifiedBsmtCond', 'BsmtCond',
        'SimplifiedBsmtCond', 'GarageQual', 'SimplifiedGarageQual', 'GarageCond', 'SimplifiedGarageCond',
        'ExterQual', 'SimplifiedExterQual', 'ExterCond', 'SimplifiedExterCond', 'HeatingQC', 'SimplifiedHeatingQC',
        'PoolQC', 'SimplifiedPoolQC', 'KitchenQual', 'SimplifiedKitchenQual', 'BsmtFinType1', 'SimplifiedBsmtFinType1',
        'BsmtFinType2', 'SimplifiedBsmtFinType2', 'Functional', 'SimplifiedFunctional', 'Fence', 'SimplifiedFence',
        'BsmtExposure', 'SimplifiedBsmtExposure', 'GarageFinish', 'LandSlope', 'LotShape', 'PavedDrive', 'Street',
        'Alley', 'CentralAir', 'MSSubClass', 'OverallCond', 'SimplifiedOverallCond', 'YrSold', 'MoSold')
lbl = LabelEncoder()
...
# Saving to CSV
pd.DataFrame({'Id': test_ID, 'SalePrice': final_labels}).to_csv('submission-16.csv', index =False)
